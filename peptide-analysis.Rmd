...
ups1_diag <- function(filename, title){
  # Raw UPS1 Data
  ups1_raw <- read.table(filename,
                     sep = "\t", comment.char="", header = TRUE) %>%
    select(proteins = Protein.s.,
           Sequence,
           validated_psms = X.Validated.PSMs)
...
ups2_diag("Data/UPS/UPS1/UPS1_04-Peptide_Shaker_on_data_24__Peptide_Report.tabular", "UPS1 Replicate 4")
