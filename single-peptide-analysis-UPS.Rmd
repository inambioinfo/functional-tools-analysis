...
### Which protein has the fewest amount of GO terms?
```{r}
ups_go_term_counts <- mutate(pepprots, count = sapply(ups_go, function(x) length(str_split(x, "; ", simplify=TRUE))))
lgt_ups <- ups_go_term_counts[ups_go_term_counts$count == min(ups_go_term_counts$count),]
lgt_ups # Least GO Terms UPS
```
