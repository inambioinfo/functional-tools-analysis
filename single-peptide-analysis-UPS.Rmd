...
```{r}
cov_pat <- "\ \\(.{2,4}\\)"
uni <- read.csv('results/unipept/UPS1_03-Unipept-20pep-1003seed.csv',
...
```{r}
length(ups) == length(intersect(ups, mg))
```
EggNOG mapper doesn't, but it seems to cover more than Unipept.
```{r}
length(ups) == length(intersect(ups, em))
exclus_buni <- setdiff(unlist(intersect(ups, buni)),
                intersect(ups, em))
exclus_em <- setdiff(unlist(intersect(ups, em)),
              intersect(ups, buni))
print("BLAST/Unipept exclusive:")
length(intersect(ups, buni))
get_go_names(exclus_buni)
exclus_buni
print("")

print("EggNOG exclusive:")
length(intersect(ups, em))
...
paths_em_extr <- data.frame(em_extraneous) #%>%
  #mutate(paths = sapply(em_extraneous, function(x) shortest_path(x, "GO:0005819")))
  #mutate(paths = sapply(em_extraneous, function(x, em_extraneous) map_paths(x, em_extraneous)))
  #mutate(paths = )
for (i in 1:length(em_extraneous)){
  #paths_em_extr$hello = 'yo'
  #colnames(paths_em_extr)[0] = 'no'
  paths_em_extr$paths[i] <- map_paths(em_extraneous[i], em_extraneous)
  #mutate(paths_em_extr,
   #      paths = map_paths(x, em_extraneous[i])) 
}

paths_em_extr
 # mutate(paths = lapply(em_extraneous, function(x) shortest_path(x, "GO:0005819")))
#paths_em_extr
#lapply(em_extraneous, function(x) shortest_path(x, "GO:0005819"))
```


```{r}
shortest_path("GO:0005819","GO:0005856")
```
...
```{r}
em_descendants
head(get_go_names(em_descendants))
head(get_go_names(mg_descendants))
```

```{r}
head(get_go_names(em_ancestors))
head(get_go_names(mg_ancestors))
```

```{r}
head(get_go_names(em_extraneous))
head(get_go_names(mg_extraneous))
```
