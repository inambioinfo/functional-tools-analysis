...
#for (n in 1:nrow(all_results)){
 if (n != 16){
  ...
  paths$em.ancestors[n] <- average_paths(em_buni, em_ancestors)
...
average_paths <- function(ref_terms, targ_terms){
  results <- sapply(targ_terms, 
              function(x) sapply(ref_terms, function(y) shortest_path(y, x)))
  print(mean(as.numeric(results[results != "NA"])))
}
hello <- average_paths(em_buni[9:10], em_ancestors[5:8])
